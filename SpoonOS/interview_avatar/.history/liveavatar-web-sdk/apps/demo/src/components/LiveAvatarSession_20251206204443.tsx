...
const LiveAvatarSessionComponent: React.FC<{
  ...
  const sendTextToCustomAvatar = async (text: string) => {
    try {
      ...
      // Stream audio to the CUSTOM session
      await sessionRef.current?.sendAudioChunk(audioBuffer);
      await sessionRef.current?.sendAudioEnded();
    } catch (err) {
      console.error("Error sending text to custom avatar:", err);
    }
  };
  ...
  onClick={() => {
    if (mode === "CUSTOM") {
      sendTextToCustomAvatar(message);
    } else {
      sendMessage(message);
    }
    setMessage("");
