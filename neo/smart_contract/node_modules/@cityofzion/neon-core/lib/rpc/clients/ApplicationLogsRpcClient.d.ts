import Query, { ApplicationLogJson } from "../Query";
import { RpcDispatcher, RpcDispatcherMixin } from "./RpcDispatcher";
export declare function ApplicationLogsRpcMixin<TBase extends RpcDispatcherMixin>(base: TBase): {
    new (...args: any[]): {
        getApplicationLog(blockOrTxHash: string): Promise<ApplicationLogJson>;
        url: string;
        execute<TResponse>(query: Query<import("../Query").JsonRpcParams, TResponse>, config?: import("./RpcDispatcher").RpcConfig): Promise<TResponse>;
        executeAll<TResponses extends unknown[]>(batchQuery: import("..").BatchQuery<import("../Query").JsonRpcParams[], TResponses>, config?: import("./RpcDispatcher").RpcConfig): Promise<TResponses>;
        executeAll<TResponses extends unknown[]>(batchQuery: Query<import("../Query").JsonRpcParams, unknown>[], config?: import("./RpcDispatcher").RpcConfig): Promise<TResponses>;
    };
} & TBase;
declare const ApplicationLogsRpcClient_base: {
    new (...args: any[]): {
        getApplicationLog(blockOrTxHash: string): Promise<ApplicationLogJson>;
        url: string;
        execute<TResponse>(query: Query<import("../Query").JsonRpcParams, TResponse>, config?: import("./RpcDispatcher").RpcConfig): Promise<TResponse>;
        executeAll<TResponses extends unknown[]>(batchQuery: import("..").BatchQuery<import("../Query").JsonRpcParams[], TResponses>, config?: import("./RpcDispatcher").RpcConfig): Promise<TResponses>;
        executeAll<TResponses extends unknown[]>(batchQuery: Query<import("../Query").JsonRpcParams, unknown>[], config?: import("./RpcDispatcher").RpcConfig): Promise<TResponses>;
    };
} & typeof RpcDispatcher;
export declare class ApplicationLogsRpcClient extends ApplicationLogsRpcClient_base {
    get [Symbol.toStringTag](): string;
}
export {};
//# sourceMappingURL=ApplicationLogsRpcClient.d.ts.map